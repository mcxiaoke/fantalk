...
public class OAuth implements java.io.Serializable {
    ...
    public String generateAuthorizationHeader(String method, String url, PostParameter[] params, String nonce, String timestamp, OAuthToken otoken) {
        List<PostParameter> oauthHeaderParams = generateAuthorizationString(
				method, url, params, nonce, timestamp, otoken);
        return "OAuth realm=\"\"," + encodeParameters(oauthHeaderParams, ",", true);
    }
    
    public String generateAuthorizationUrl(String method, String url, PostParameter[] params, String nonce, String timestamp, OAuthToken otoken) {
        List<PostParameter> oauthHeaderParams = generateAuthorizationString(
				method, url, params, nonce, timestamp, otoken);
        return encodeParameters(oauthHeaderParams, "&", false);
    }

	private List<PostParameter> generateAuthorizationString(String method,
			String url, PostParameter[] params, String nonce, String timestamp,
			OAuthToken otoken) {
		if (null == params) {
     ...
     * 閳ワ拷Parameters in the OAuth HTTP Authorization header excluding the realm parameter.<br>
     * 閳ワ拷Parameters in the HTTP POST request body (with a content-type of application/x-www-form-urlencoded).<br>
     * 閳ワ拷HTTP GET parameters added to the URLs in the query part (as defined by [RFC3986] section 3).<br>
     * <br>
     ...
     * 3.	<br>
     * 4.	Parameters are concatenated in their sorted order into a single string. For each parameter, the name is separated from the corresponding value by an 閳ワ拷閳ワ拷character (ASCII code 61), even if the value is empty. Each name-value pair is separated by an 閳ワ拷閳ワ拷character (ASCII code 38). For example:<br>
